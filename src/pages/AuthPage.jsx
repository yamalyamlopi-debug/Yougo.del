...
import { C, IcoCheck, IcoClose, IcoBack, IcoFork, IcoStore, IcoTruck, IcoShield, IcoUser, IcoEdit } from "../components/Icons";
    ...
    <div style={{ fontFamily: "Arial,sans-serif", background: C.bg, minHeight: "100vh", maxWidth: 430, margin: "0 auto", direction: "rtl", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ flex: 1, padding: "30px 24px" }}>
        ...
        <div style={{ display: "flex", gap: 10, marginBottom: 4 }}>
          <div style={{ background: C.white, border: "1.5px solid " + C.lightGray, borderRadius: 14, padding: "13px 14px", display: "flex", alignItems: "center", gap: 6, flexShrink: 0 }}>
          ...
          <input value={phone} onChange={e => { setPhone(e.target.value); setPhoneError(""); }}
            ...
            style={{ flex: 1, background: C.white, border: "1.5px solid " + (phoneError ? C.red : C.lightGray), borderRadius: 14, padding: "13px 16px", fontSize: 16, outline: "none", direction: "ltr", textAlign: "left", fontFamily: "Arial,sans-serif", color: C.dark }} />
        </div>
        {phoneError && <div style={{ color: C.red, fontSize: 12, marginBottom: 12 }}>{phoneError}</div>}
        ...
        <button onClick={handlePhoneSubmit} disabled={loading}
          ...
          {loading ? <><LoadSpinner /> שולח קוד...</> : <><IcoCheck s={18} /> המשך</>}
        </button>
        ...
        <button onClick={() => { setLoading(true); setTimeout(() => { setLoading(false); onDone({ name: "משתמש Google", phone: phone || "050-0000000", authMethod: "google" }); }, 1000); }}
          style={{ width: "100%", background: "white", color: C.dark, border: "1.5px solid " + C.lightGray, borderRadius: 14, padding: "13px", fontSize: 14, fontWeight: 700, cursor: "pointer", marginBottom: 10, display: "flex", alignItems: "center", justifyContent: "center", gap: 10 }}>
        ...
        <button onClick={() => { setLoading(true); setTimeout(() => { setLoading(false); onDone({ name: "משתמש Apple", phone: phone || "050-0000000", authMethod: "apple" }); }, 1000); }}
        ...
        <div style={{ display: "flex", gap: 10, justifyContent: "center", marginBottom: 8, direction: "ltr" }}>
          {otp.map((digit, idx) => (
            <input key={idx} id={"otp-" + idx} value={digit}
              ...
              maxLength={1}
              style={{ width: 52, height: 60, textAlign: "center", fontSize: 24, fontWeight: 900, border: "2px solid " + (otpError ? C.red : digit ? C.red : C.lightGray), borderRadius: 14, outline: "none", background: digit ? "rgba(200,16,46,0.05)" : C.white, color: otpError ? "#EF4444" : C.dark, fontFamily: "Arial,sans-serif" }} />
          ))}
        </div>
        ...
        <div style={{ flex: 1, padding: "28px 24px", overflowY: "auto" }}>
            ...
            {[{ l: "שם פרטי", k: "firstName", ph: "כגון: חמדאן" }, { l: "שם משפחה", k: "lastName", ph: "כגון: אחמד" }].map(f => (
              <div key={f.k} style={{ flex: 1 }}>
                ...
                <input value={form[f.k]} onChange={e => { const v = e.target.value; setForm(p => ({ ...p, [f.k]: v })); setFormErrors(p => ({ ...p, [f.k]: "" })); }}
                  placeholder={f.ph}
                  style={{ width: "100%", background: C.white, border: "1.5px solid " + (formErrors[f.k] ? C.red : C.lightGray), borderRadius: 13, padding: "12px 13px", fontSize: 13, outline: "none", direction: "rtl", fontFamily: "Arial,sans-serif" }} />
            ...
            <div style={{ display: "flex", gap: 10 }}>
              {[{ v: "male", l: "זכר" }, { v: "female", l: "נקבה" }, { v: "other", l: "אחר" }].map(g => (
                <button key={g.v} onClick={() => { setForm(p => ({ ...p, gender: g.v })); setFormErrors(p => ({ ...p, gender: "" })); }}
                  style={{ flex: 1, padding: "12px 8px", borderRadius: 14, border: "2px solid " + (form.gender === g.v ? C.red : C.lightGray), background: form.gender === g.v ? "rgba(200,16,46,0.06)" : C.white, cursor: "pointer", fontSize: 12, fontWeight: form.gender === g.v ? 700 : 500, color: form.gender === g.v ? C.red : C.gray }}>
                  {g.l}
                </button>
              ))}
            </div>
          ...
          <div style={{ marginBottom: 24 }}>
            ...
            <input value={form.age} onChange={e => { const v = e.target.value.replace(/\D/g, ""); setForm(p => ({ ...p, age: v })); setFormErrors(p => ({ ...p, age: "" })); }}
              ...
              style={{ width: "100%", background: C.white, border: "1.5px solid " + (formErrors.age ? C.red : C.lightGray), borderRadius: 13, padding: "12px 13px", fontSize: 14, outline: "none", direction: "rtl", fontFamily: "Arial,sans-serif" }} />
            {formErrors.age && <div style={{ color: C.red, fontSize: 10, marginTop: 3 }}>{formErrors.age}</div>}
          </div>
          <button onClick={handleRegister} disabled={loading}
            style={{ width: "100%", background: loading ? "rgba(200,16,46,0.5)" : C.red, color: "white", border: "none", borderRadius: 16, padding: "15px", fontSize: 15, fontWeight: 900, cursor: loading ? "not-allowed" : "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 10, boxShadow: "0 6px 20px rgba(200,16,46,0.35)" }}>
            {loading ? <><LoadSpinner /> יוצר חשבון...</> : <><IcoCheck s={18} /> יצירת חשבון</>}
          </button>
        </div>
        <style>{`*{box-sizing:border-box}`}</style>
      </div>
